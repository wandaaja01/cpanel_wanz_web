...
export default async function handler(req, res) {
  ...
  try {
    const configRes = await fetch("https://raw.githubusercontent.com/wandaaja01/panel-settings/main/premium-users.json");
    const config = await configRes.json();

    const { domain, plta, pltc, egg, nestid, location } = config;

    const dbRes = await fetch("https://raw.githubusercontent.com/wandaaja01/panel-settings/main/users.json");
